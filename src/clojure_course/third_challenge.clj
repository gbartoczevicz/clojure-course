...
(defn is-valid-operation-type
  ...
  (if-not (or (= type "buy") (= type "sell") (= type "initial"))
    (do (throw (IllegalArgumentException. (format "The type '%s' is invalid" type))))))

(defn is-valid-amount
  [^Integer amount]
  (if (< amount 0)
    (do (throw (IllegalArgumentException. (format "The amount '%d' needs to be positive" amount))))))
...
(defn extract-raw-value-from-operation
  [operation]
  (def op-type (:type operation))
  (def op-amount (:amount operation))
...
(defn sum-operation-seq-amount
  [operation-seq]
  (def to-reduce-amount-seq (map #(extract-raw-value-from-operation %) operation-seq))
  (r/fold + to-reduce-amount-seq))

(defn can-purchase-item
  [accounts item]
  (def buyer-operations (:buyer accounts))
  (def total-amount (sum-operation-seq-amount buyer-operations))
  (def if-buying-after-value (- total-amount (:amount item)))
  (>= if-buying-after-value 0))

(defn create-accounts-from-incoming-item
  [accounts, item]
  (def item-amount (:amount item))

  (def buy-operation (create-operation "buy" item-amount))
  (def sell-operation (create-operation "sell" item-amount))

  (def buyer-operations (conj (:buyer @accounts) buy-operation))
  (def merchant-operations (conj (:merchant @accounts) sell-operation))

  (create-accounts-with-operations buyer-operations merchant-operations))

(defn get-random-item
  []
  (def available-items (create-available-items))
  (def size (count available-items))
  (def position (rand-int size))
...
(defn purchase-item
  ...
  (def updated-accounts (create-accounts-from-incoming-item accounts item))
  (ref-set accounts updated-accounts)

  (def updated-items-purchased (create-purchased-items-from-item items-purchased item))
...
(defn simulate
  ...
  (def item-name (:name item))

  (cond
    (can-purchase-item @accounts item) (do
                                         (purchase-item accounts items-purchased item)
                                         (print-item-deal position item-name "purchased :)"))
    :else (print-item-deal position item-name "not purchased :(")))
...
(defn print-items-summary
  ...
  (def to-sum-amount (map #(extract-raw-value-from-item %) items))
  (println "Total: R$" (r/fold + to-sum-amount)))

(defn -main
  "Simple selling system"
  []
  (def accounts (ref (create-accounts-with-operations
                       [(create-operation "initial" 100)]
                       [(create-operation "initial" 0)])))
  (def items-purchased (ref []))

  (def iterations (rand-int 20))
